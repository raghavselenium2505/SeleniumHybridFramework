...
public class DynamicDataProvider {
    ...
    public Object[][] getData(Method method) throws Exception {
        ...
        throw new RuntimeException(
                "No valid group defined for test: "
                        + method.getName());
    }
    ...
    private Object[][] getJsonData(String testName) throws Exception {
        ...
        for (Object obj : jsonArray) {
            ...
            String testCase =
                    jsonObject.get("testCase").toString();

            String runMode =
                    jsonObject.get("runMode").toString();

            if (testCase.equalsIgnoreCase(testName)
                    && runMode.equalsIgnoreCase("yes")) {
        ...
        if (dataList.isEmpty()) {
            throw new RuntimeException(
                    "No JSON data found for: " + testName);
        }
    ...
    private Object[][] getExcelData(String testName) {
        ...
        ExcelUtil excelUtil =
                new ExcelUtil(filePath);

        Object[][] data =
                excelUtil.getSheetDataByTestCaseName(
                        "Signin",
                        testName);

        if (data.length == 0) {
            throw new RuntimeException(
                    "No Excel data found for: "
                            + testName);
        }
