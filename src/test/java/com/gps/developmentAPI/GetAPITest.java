...
import com.gps.baseAPI.GetAPIExecutor;
...
public class GetAPITest extends APITestBase {
    ...
    public List<Map<String, String>> getExcelData() {
        return new ExcelUtil(System.getProperty("user.dir")
                ...
                .getDataAsMap("RunManagerAPI", "getAPITest");
    }
    ...
    @Test
    @Parameters({"dataMode"})
    public void runGetTests(@Optional("") String mode) {
        ...
        GetAPIExecutor api = new GetAPIExecutor();
        ...
        if ("excel".equals(finalMode)) {
            ...
            for (Map<String, String> row : dataList) {
                ...
                if (method == null || !method.equalsIgnoreCase("GET")) continue;
        ...
        // ================= JSON =================
            ...
            for (Object obj : arr) {
                ...
                if (!"GET".equalsIgnoreCase(method)) continue;
        ...
        if (!isExecuted) {
            throw new SkipException("No GET tests executed for mode: " + finalMode);
        }
    }
}